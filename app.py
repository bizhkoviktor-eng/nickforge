...
def init_db():
    ...
    con.execute("CREATE TABLE IF NOT EXISTS nicknames (nickname TEXT PRIMARY KEY, created_at TEXT NOT NULL)")
    con.commit()
    con.close()

def clean(value):
    return re.sub(r"[^A-Za-z0-9]", "", value or "")[:15]

def make_name(style, word="", separators=False, numbers=False):
    title, prefixes, suffixes = STYLES.get(style, STYLES["cyber"])
    word = clean(word)
    p, s = random.choice(prefixes), random.choice(suffixes)
    choices = [
    ...
    if separators and len(result) > 5:
        pos = max(1, len(result)//2)
        result = result[:pos] + "_" + result[pos:]
    if numbers and random.random() < .6:
        result += str(random.randint(1,999))
    return result[:20]

def unique_name(style, word, separators, numbers):
    con = sqlite3.connect(DB)
    for _ in range(100):
        name = make_name(style, word, separators, numbers)
        if not con.execute("SELECT 1 FROM nicknames WHERE nickname=?", (name,)).fetchone():
            con.execute("INSERT INTO nicknames VALUES (?,?)", (name, datetime.utcnow().isoformat()))
            con.commit()
            con.close()
            return name
    con.close()
    return make_name(style, word, separators, True)
...
def generate():
    ...
    separators = bool(data.get("separator"))
    numbers = bool(data.get("numbers"))
    names = []
    while len(names) < count:
        name = unique_name(style, word, separators, numbers)
...
<url><loc>https://nickforge.onrender.com/</loc><changefreq>weekly</changefreq><priority>1.0</priority></url>
